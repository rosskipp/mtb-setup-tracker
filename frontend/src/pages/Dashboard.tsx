    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Your riding overview</p>
        </div>
        <Link to="/rides/new" className="btn-primary">+ Log Ride</Link>
      </div>
          ...
          <div className="text-2xl font-bold mt-1">
            {rides.filter((r) => r.rating).length > 0
              ? (rides.filter((r) => r.rating).reduce((sum, r) => sum + (r.rating || 0), 0) /
                  rides.filter((r) => r.rating).length).toFixed(1)
              : '-'}
          </div>
        </div>
        <div className="card p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {bikes.map((bike) => (
              <div key={bike.id} className="card p-4 hover:border-brand-500/50 transition-colors">
                ...
                {(bike.travel_front_mm || bike.travel_rear_mm) && (
                  <div className="flex gap-3 mt-3 text-xs text-gray-500 dark:text-gray-400">
                    {bike.travel_front_mm && <span>Fork: {bike.travel_front_mm}mm</span>}
                    {bike.travel_rear_mm && <span>Rear: {bike.travel_rear_mm}mm</span>}
                  </div>
                )}
              </div>
            ))}
      ...
      <section>
        ...
        {rides.length === 0 ? (
          <div className="card p-8 text-center">
            <p className="text-gray-500 dark:text-gray-400 mb-4">No rides logged yet. Get out there!</p>
            <Link to="/rides/new" className="btn-primary">Log Your First Ride</Link>
          </div>
        ) : (
          <div className="space-y-3">
            {rides.map((ride) => (
              <Link key={ride.id} to={`/rides/${ride.id}/edit`}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {ride.trail_condition && (
                        <span className="text-lg" title={ride.trail_condition}>
                          {CONDITIONS_EMOJI[ride.trail_condition] || '\ud83c\udfd4\ufe0f'}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    ...
                    {ride.setup.front_tire_pressure_psi && (
                      <span className="badge bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300">
                        F: {ride.setup.front_tire_pressure_psi} / R: {ride.setup.rear_tire_pressure_psi} psi
                      </span>
                    )}
                  </div>
                )}
      ...
      {bikes.length === 0 && (
        <div className="card p-8 text-center">
          <p className="text-gray-500 dark:text-gray-400 mb-4">Add a bike first to start tracking setups</p>
          <Link to="/bikes" className="btn-primary">Add a Bike</Link>
        </div>
      )}
    </div>
  );
